...
import ru.romanov.outbox.configuration.OutboxProperties;
...
public class OutboxMessageRepositoryImpl implements OutboxMessageRepository {
    ...
    private static final String INSERT_SQL =
            "INSERT INTO %s "
                    + "(id, key, payload, headers, "
                    + "topic, kafka_system, create_time, reserved_to, status) "
                    + "VALUES (:id, :key, :payload::json, :headers::jsonb, :topic, :kafka_system, :create_time, :reserved_to, :status)";
    ...
    private static final String DELETE_PROCESSED_MESSAGES_SQL =
            "DELETE FROM %s "
                    + "WHERE id IN("
                    + "SELECT id FROM %s "
                    + "WHERE create_time < CURRENT_DATE - :daysInterval "
                    + "AND status = 'SUCCESS' LIMIT :limit FOR UPDATE SKIP LOCKED)";

    private static final String SELECT_STUCK_MESSAGES_SQL =
            "SELECT * FROM %s "
    ...
    public OutboxMessageEntity save(OutboxMessageEntity entity) {
        ...
        jdbcTemplate.update(String.format(INSERT_SQL, properties.getTableName()), params);
        return entity;
    }
    ...
    public void updateState(OutboxMessageEntity entity) {
        ...
        jdbcTemplate.update(String.format(UPDATE_SQL, properties.getTableName()), params);
    }

    @Override
    public void deleteProcessedMessages(Integer daysInterval, Integer limit) {
        MapSqlParameterSource params =
                new MapSqlParameterSource().addValue("daysInterval", daysInterval).addValue("limit", limit);
        var tableName = properties.getTableName();
    ...
    public List<OutboxMessageEntity> findStuckMessages(Integer limit) {
        MapSqlParameterSource params =
                new MapSqlParameterSource()
                        ...
                        .addValue(
                                "statuses", List.of(OutboxMessageStatus.NEW.name(), OutboxMessageStatus.FAILURE.name()));

        return jdbcTemplate.query(
                String.format(SELECT_STUCK_MESSAGES_SQL, properties.getTableName()),
                params,
                (rs, rowNum) ->
